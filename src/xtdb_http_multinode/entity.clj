...
(defn search-entity-history [xtdb-node {:keys [eid valid-time tx-time tx-id sort-order with-corrections with-docs
...
(defn search-entity [xtdb-node {:keys [eid valid-time tx-time tx-id link-entities?]}]
  (try
    (let [db (util/db-for-request xtdb-node {:valid-time valid-time
                                             :tx-time tx-time
                                             :tx-id tx-id})
...
(defn entity-state [req]
  (let [{:keys [eid history] :as query-params} (transform-query-params req)
        xtdb-node (get req :xtdb-node)]
    (-> (cond
          (nil? eid) {:no-entity? true}
          history (search-entity-history xtdb-node query-params)
          :else (search-entity xtdb-node query-params))
